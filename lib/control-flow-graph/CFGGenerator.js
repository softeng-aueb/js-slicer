...
class CFGGenerator {
    ...
    static constructBBCfg(cfg) {
        ...
        // First statement of a program is always a leader, start first Basic block
        const firstNode = cfg.getNodeById(1);
        currentBB.addNode(firstNode);
        BBcfg.addNode(currentBB);

        let traversalNodes = [...firstNode.nextNodes([])];
        let nodesFound = [firstNode, ...firstNode.nextNodes([])];

        if (cfg.isJumpTargetNode(firstNode)) {
        ...
        while (traversalNodes.length > 0) {
            ...
            if (cfg.isJumpTargetNode(currentNode)) {
                if (currentBB.nodes.length > 0) currentBB = new BasicBlock(BBId++);
        ...
        for (let bb of BBcfg.nodes) {
            for (let otherBB of BBcfg.nodes) {
                if (bb != otherBB) {
                    let edgesConditions = bb.hasEdgeToBlock(otherBB);
                    if (edgesConditions.length > 0) for (let edgeCond of edgesConditions) bb.addOutgoingEdge(otherBB, edgeCond);
                }
            }
        }
