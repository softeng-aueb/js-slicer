...
class AstObjectTypesParser {
    static expressionParser(expressionAstObj) {
        ...
        switch (expressionAstObj.type) {
            ...
            case AST_OBJECT_TYPES.TRY_STATEMENT:
                return this.tryStatementParser(expressionAstObj);
    ...
    static catchClauseParser(catchClauseAstObj) {
        let body = this.expressionParser(catchClauseAstObj.body);
        let param = catchClauseAstObj.param ? this.expressionParser(catchClauseAstObj.param) : null;

        return new CatchClause(param, body);
    }

    static tryStatementParser(tryStatementAstObj) {
        let block = this.expressionParser(tryStatementAstObj.block);
        let handler = tryStatementAstObj.handler ? this.expressionParser(tryStatementAstObj.handler) : null;
        let finalizer = tryStatementAstObj.finalizer ? this.expressionParser(tryStatementAstObj.finalizer) : null;
    ...
    static switchCaseParser(switchCaseAstObj, discriminant) {
        ...
        let test =
            switchCaseAstObj.test !== null
                ...
                : null;
        let consequent =
            switchCaseAstObj.consequent[0]?.type === AST_OBJECT_TYPES.BLOCK_STATEMENT
                ? switchCaseAstObj.consequent[0]
                : { type: AST_OBJECT_TYPES.BLOCK_STATEMENT, body: switchCaseAstObj.consequent };
    ...
    static ifStatementParser(ifStatementObj) {
        ...
        let then =
            ifStatementObj.consequent.type === AST_OBJECT_TYPES.BLOCK_STATEMENT
                ? ifStatementObj.consequent
                : { type: AST_OBJECT_TYPES.BLOCK_STATEMENT, body: [ifStatementObj.consequent] };
        then = this.blockStatementParser(then);

        let alternates = null;
        if (ifStatementObj.alternate) {
            alternates =
                ifStatementObj.alternate.type === AST_OBJECT_TYPES.BLOCK_STATEMENT
                    ? ifStatementObj.alternate
                    : { type: AST_OBJECT_TYPES.BLOCK_STATEMENT, body: [ifStatementObj.alternate] };
            alternates = this.blockStatementParser(alternates);
        }
        return new ConditionalStatement(condition, then, alternates);
    }
    ...
    static loopStatementParser(loopStatementAstObj) {
        ...
        let body = this.expressionParser(
            loopStatementAstObj.body.type === AST_OBJECT_TYPES.BLOCK_STATEMENT
                ? loopStatementAstObj.body
                : { type: AST_OBJECT_TYPES.BLOCK_STATEMENT, body: [loopStatementAstObj.body] }
        );
