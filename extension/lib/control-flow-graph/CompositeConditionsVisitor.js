...
class CompositeConditionsVisitor {
    ...
    visit(root, returnDecisionNode = true) {
        ...
        while (this._postOrderNodeQueue.length > 0) {
            ...
            if (!CFGNodeLastVisited) {
                CFGNodeLastVisited = currentNode;
            } else if (operatorQueue.length > 0) {
                let previousNode = CFGNodeLastVisited;
                CFGNodeLastVisited = null;
                ...
                if (operatorSym === "||") {
                    ...
                    // if there is no other next node, add to true list
                    let copyNodeQueue = [...this._postOrderNodeQueue];
                    let copyOperatorQueue = [...operatorQueue];
                    let nextNode = this.findNextNode(copyNodeQueue, copyOperatorQueue, "&&", operatorLvl);

                    // If no next node is found on the same level or higher add to true list
                    if (!nextNode) {
                    ...
                    // if there is no other next node add to false list

                    let copyNodeQueue = [...this._postOrderNodeQueue];
                    let copyOperatorQueue = [...operatorQueue];
                    let nextNode = this.findNextNode(copyNodeQueue, copyOperatorQueue, "||", operatorLvl);
                    // If no next node is found on the same level or higher add to false list
                    if (!nextNode) {
                        if (this._compositeNodes.includes(previousNode)) {
    ...
    findNextNode(copyNodeQueue, copyOperatorQueue, operator, operatorLvl) {
        let nextNode = null;
        while (copyNodeQueue.length > 0) {
            ...
            if (copyOp.level <= operatorLvl && copyOp.operator === operator) {
                nextNode = temp;
                break;
            }
        }
